...
class Peptide(object):
    """
        ...
        # atom position list (顺序: N, CA, C, O)
        pos_N
        pos_CA
        pos_C
        pos_O
    ...
    def __init__(self, pos_list, fasta):
        # 修改：采用标准原子顺序 [N, CA, C, O]
        self.pos_N = pos_list[:, 0, :]   # 第0维 → N
        self.pos_CA = pos_list[:, 1, :]  # 第1维 → CA
        self.pos_C = pos_list[:, 2, :]   # 第2维 → C
        self.pos_O = pos_list[:, 3, :]   # 第3维 → O
        self.fasta = fasta
        self.structure = None
    ...
    def output_to_pdb(self, pdb_path):
        """
        保存结构为PDB文件
        pdb_path: 完整的文件路径，例如 "data/output/structures/MMCD_0006.pdb"
        """
        # 确保目录存在
