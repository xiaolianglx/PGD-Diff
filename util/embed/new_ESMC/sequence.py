...
def load_esmc_embeddings(json_path="data/esmc_embeddings_complete.json"):
    """从JSON文件加载预计算的ESMC嵌入"""
    ...
    try:
        ...
        print(f"成功加载 {len(ESMC_EMBEDDINGS)} 个序列的ESMC嵌入")
        return ESMC_EMBEDDINGS
    except Exception as e:
        print(f"加载ESMC嵌入失败: {str(e)}")
        ESMC_EMBEDDINGS = {}
        return {}

def get_esmc_embedding_from_peptide(fasta):
    """通过序列内容获取ESMC嵌入"""
    global ESMC_EMBEDDINGS
    ...
    if fasta in ESMC_EMBEDDINGS:
        embedding = np.array(ESMC_EMBEDDINGS[fasta])
        return embedding
    
    # 如果没有找到，返回适当维度的零向量
    #print(f"警告: 未找到序列 '{fasta[:20]}...' 的ESMC嵌入")
...
def get_embedding_form_peptide(fasta=None, encode_type=None):
    # 添加ESMC选项
    assert encode_type in {"pssm", "blosum", "pam", "hydrophobicity", "esmc"}, "embedding type error"
    ...
    if encode_type == "pssm":
        ...
        # 关键修改: 直接使用序列内容获取ESMC嵌入
        embedding = get_esmc_embedding_from_peptide(fasta)
        if embedding is None:
            # 返回零向量作为后备
            embedding = np.zeros((len(fasta), 1152))
...
def get_bio_embedding_for_sequence(fasta, encode_type=None):
    ...
    for aa_type in encode_type:
        ...
        if isinstance(emb, list):
            emb = np.array(emb)
        if emb.ndim == 1:
            emb = emb.reshape(len(fasta), -1)
        
        embedding_list.append(emb)
    
    # 确保所有特征长度一致
    min_len = min([emb.shape[0] for emb in embedding_list]) if embedding_list else len(fasta)
    embedding_list = [emb[:min_len] for emb in embedding_list]
    
    # 合并所有特征
    if embedding_list:
        embedding = np.concatenate(embedding_list, axis=1)
    else:
        embedding = np.zeros((min_len, 0))
    
    return embedding

# 在文件末尾添加以下函数

def get_bio_embedding_for_sequence_generation(fasta, encode_type=None):
    """
    专门用于生成阶段的生物特征嵌入函数
    不使用ESMC嵌入，减少计算开销
    """
