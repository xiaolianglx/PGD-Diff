...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, batch, pair_bias=None, extra_kv=None):
        """
        x: (total_len, n_emb)
        batch: (total_len,) 每个 token 所属样本的索引
        pair_bias: list of (L_i, L_i) tensors
        extra_kv: list of tuples (extra_k_i, extra_v_i) for each sample,
                  each extra_k_i shape (prefix_len, n_emb), same for v
        """
        ...
        for i in range(n_samples):
            ...
            # 拼接额外的前缀 Key/Value（如果存在）
            if extra_kv is not None and i < len(extra_kv):
                extra_k_i, extra_v_i = extra_kv[i]   # each (prefix_len, n_emb)
                if extra_k_i is not None:
