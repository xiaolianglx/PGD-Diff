...
class SeqBlock(nn.Module):
    ...
    def forward(self, x, time_step, batch, struct_emb=None, pair_bias=None, cancer_idx=None):
        # 时间步和位置编码
...
class SeqTransformer(nn.Module):
    ...
    def forward(self, x, time_step, batch=None, struct_emb=None, cond_emb=None, pair_bias=None, cancer_idx=None):
        ...
        if cond_emb is not None:
            x_emb = x_emb + cond_emb[batch]
        for block in self.blocks:
            x_emb, _ = block(x_emb, time_step, batch, struct_emb, pair_bias, cancer_idx=cancer_idx)
