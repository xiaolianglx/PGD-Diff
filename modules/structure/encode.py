...
#为每个 time_step 编码时序特征，生成类似 transformer positional encoding 的嵌入。
...
class MLP_block(nn.Module):
    def __init__(self, input_dim, hidden_dim, activation="relu", dropout=0.1, num_block=32):
        ...
        self.time_embed = TimeEncoder(hidden_dim * 2)#使用 TimeEncoder 增强时序信息
...
#为边构造一组详细的原子对特征（Atom Pair Features）
def atom_pair_feature(node_attr, edge_index, edge_attr, pos):#为边构造一组详细的 原子对特征（Atom Pair Features）
...
#get_backbone_atom_embedding() 返回 4 种 backbone 原子的 one-hot（如 N, CA, C, O）

#利用 structure_embedding 提供结构输入，包括节点、边、边特征、边距离等。
